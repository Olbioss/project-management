...
export default function BoardView({ id, setIsModalNewTaskOpen }: Props) {
  const {
    data: tasks,
    isLoading,
    error,
  ...
  const moveTask = (taskId: number, toStatus: string) => {
    updateTask({ taskId, status: toStatus as Status });
  };
...
const TaskColumn = ({
  ...
  const [{ isOver }, drop] = useDrop(() => ({
    ...
    collect: (monitor: DropTargetMonitor<{ id: number }>) => ({
      ...
      <div className="mb-3 flex w-full">
        ...
        <div className="flex w-full items-center justify-between rounded-e-lg bg-white px-5 py-4 dark:bg-dark-secondary">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              className="flex h-6 w-6 items-center justify-center rounded-sm bg-gray-200 dark:bg-dark-tertiary dark:text-white"
...
const Task = ({ task }: TaskProps) => {
  const [{ isDragging }, drag] = useDrag(() => ({
    type: "task",
    item: { id: task.id },
    collect: (monitor: DragSourceMonitor<{ id: number }>) => ({
